...
def define_cutoff_ratio(num_subbands, num_taps, beta):
    ...
    ret = optimize.minimize(_objective, np.array([0.01]), bounds=optimize.Bounds(0.01, 0.99))
    opt_cutoff_ratio = ret.x[0]
    
    return opt_cutoff_ratio
...
class PQMF(torch.nn.Module):
    ...
    def __init__(self, subbands=4, taps=62, beta=9.0, cutoff_ratio=0.142):
        ...
        cutoff_ratio = round(cutoff_ratio, 3)
...
if __name__ == "__main__":
    ...
    sf.write(f"aa.wav", x_, sr, "PCM_16")
    sf.write(f"bb-{pqmf.subbands}-{pqmf.taps}-{pqmf.beta}-{pqmf.cutoff_ratio}.wav", y_, sr, "PCM_16")
    for i in range(xs.size(1)):
        sf.write("aa{}.wav".format(i+1), xs[0,i,:].numpy().flatten(), sr//n_band, "PCM_16")
    sf.write(f"aa-m.wav", xs[0].numpy().T, sr//n_band, "PCM_16")
